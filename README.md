...
2. If changes have been made to the petalinux project or it has not been built 
   yet since cloning this repo, the following must be done:
    ...
    5. Close the terminal 
           ...
           block ram was added to an AXI GP port, or if you want to rebuild the FreeRTOS 
           include files, then Xilinx SDK should be launched from the vivado project (prior 
           to closing Vivado). The hw_handoff folder should be used as the exported hardware 
           location and the contents of the sdk folder should be deleted and then it should 
           be used as the workspace. Then hello world applications will need to be generated 
           with the names hello_rt and hello_sa (with the autogenerated BSPs too), one for 
           standalone and one for freeRTOS. This is solely to generate the lscript.ld files 
           and FreeRTOS include files used in the SDSoC platform. Finally, open the lscript.ld 
           files in each of the hello world projects and change the heap size to 0x10000000 
           and the stack size to 0x40000. Save the files, then Xilinx SDK and Vivado can be 
           closed.
...
5. Copy the README.txt file to the platform:

        cp sdsoc/README.txt sdsoc/zybo_z7_20/export/

6. Run the following to archive the platform:

        cd sdsoc/zybo_z7_20/export/
        zip -r -9 SDSoC-Zybo-Z7-20-v20XX.X-Y.zip ./zybo_z7_20 ./README.txt
    
   Where 20XX.X is the SDx version and Y is the release number. 
   
7. cd back to the repo's root directory, ensure that all changes are commited, 
   and push the commits to github.

8. Create a github release that targets the most recent commit and attach 
   the sdsoc/zybo_z7_20/export/SDSoC-Zybo-Z7-20-v20XX.X-Y.zip file to it.
